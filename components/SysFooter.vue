<template>
  <div class="bg-black w-full sm:h-20 md:h-auto flex justify-center py-5">
    <nav>
      <ul class="flex flex-col sm:flex-row text-md text-white font-bold px-5 whitespace-nowrap space-y-5 sm:space-y-0 sm:space-x-16">
        <li><RouterLink to="/impressum">Impressum</RouterLink></li>
        <li><RouterLink to="/datenschutz">Datenschutz</RouterLink></li>
        <li><RouterLink to="/contact">Kontakt</RouterLink></li>
        <li><RouterLink to="/about-us">Über uns</RouterLink></li>
      </ul>
    </nav>
  </div>
</template>
  